...
class Sarsa(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for i in range(self.options.steps): # steps-1?
            n,r,done,_ = self.step(a)
            a_prime = np.random.choice(np.arange(self.env.action_space.n),p=self.epsilon_greedy_action(n))
            self.Q[state][a] = self.Q[state][a] + self.options.alpha * (r + self.options.gamma * self.Q[n][a_prime] - self.Q[state][a])
            if done: break
            state = n
            a = a_prime
