...
class PolicyIteration(AbstractSolver):
    ...
    def train_episode(self):
        ...
        self.policy_eval() # changes V values
        ...
        for s in range(self.env.observation_space.n):
            ...
            ################################
            self.policy[s,np.argmax(self.policy[s])] = 0.0
            self.policy[s,np.argmax(self.one_step_lookahead(s))] = 1.0
    ...
    def policy_eval(self):
        ...
        for state in range(self.env.observation_space.n):
            A.append([0]*self.env.observation_space.n)
            A[-1][state] = -1
            B.append(0)
            a = np.argmax(self.policy[state])
            for prob, next_state, reward, done in self.env.P[state][a]:
                A[-1][next_state] += prob*self.options.gamma
                B[-1] -= prob*reward
