...
class DQN(AbstractSolver):
    ...
    def epsilon_greedy(self, state):
        ...
        Use:
            ...
            torch.as_tensor(state): Convert Numpy array ('state') to a tensor
    ...
    def compute_target_values(self, next_states, rewards, dones):
        ...
        #   YOUR IMPLEMENTATION HERE   #
        ################################
        # y = torch.as_tensor(np.ones(len(next_states)))
        # for j in enumerate(next_states):
        #     if dones[j]: y[j] = rewards[j]; continue
        #     y[j] = rewards[j] + self.options.gamma * torch.max(self.target_model(next_states[j]))
        # return y
        with torch.no_grad():
    ...
    def train_episode(self):
        ...
        for i in range(self.options.steps):
            ...
            a = np.random.choice(np.arange(self.env.action_space.n),p=probs)
            # execute a and observe r and x(t+1)
            n,r,done,_ = self.step(a)
            # preprocess data (collect state info from recent states) and store transition
            self.memorize(state, a, r, n, done)
            ### J gradient descent
            # sample and compare to minibatch of other trasitions from D (memory)
            # scoring (y=)
            # GD
            self.replay()

            # reset Q every C steps
            if done: break
            if not i % self.options.update_target_estimator_every: self.update_target_model()
            state = n
