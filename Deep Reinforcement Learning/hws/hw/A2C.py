...
class A2C(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            # an episode.                  # 
            ################################
            a,prob,value = self.select_action(state)
            n,r,done,_ = self.step(a)
            with torch.no_grad():
                _,vprime = self.actor_critic(torch.as_tensor(n,dtype=torch.float32))
                adv = r + (1-done) * self.options.gamma * vprime - value
            # print(f'/V={value}\tV\'={vprime}\tAdv={adv}')
            self.update_actor_critic(adv,prob,value)
            if done:break
            state = n
    ...
    def actor_loss(self, advantage, prob):
        ...
        #   YOUR IMPLEMENTATION HERE   #
        ################################
        return advantage * -torch.log(prob) # derv of a prob is ln
